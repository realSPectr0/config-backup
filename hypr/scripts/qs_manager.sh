...
route_nocturne_notifications() {
    local method="$1"

    quickshell ipc -p "$HOME/.config/quickshell/bar" \
        call qs-bar-notifications "$method" >/dev/null 2>&1
}

if [[ "$TARGET" == "notifications" && "$ACTION" =~ ^(open|toggle|close)$ ]]; then
    route_nocturne_notifications "$ACTION" && exit 0
fi
